...
public class CORStripes extends Configured implements Tool {
	...
	public static class CORStripesMapper2 extends Mapper<LongWritable, Text, Text, MapWritable> {
		private static final Text LEFT_WORD = new Text();
		...
		protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			...
			for (int i = 0; i < words.size(); i++) {
				...
				if (stripe.size() > 0) {
					LEFT_WORD.set(words.get(i));
					context.write(LEFT_WORD, stripe);
				}
			}
		}
	}
		...
		protected void reduce(Text key, Iterable<MapWritable> values, Context context) throws IOException, InterruptedException {
			...
			for (MapWritable stripe : values) {
				for (Map.Entry<Writable, Writable> entry : stripe.entrySet()) {
					Text right = (Text) entry.getKey();
					int delta = ((IntWritable) entry.getValue()).get();

					IntWritable current = (IntWritable) sumStripe.get(right);
					int newValue = (current == null ? 0 : current.get()) + delta;
					sumStripe.put(new Text(right.toString()), new IntWritable(newValue));
				}
			}
	...
	public static class CORStripesReducer2 extends Reducer<Text, MapWritable, PairOfStrings, DoubleWritable> {
		...
		private static final PairOfStrings WORD_PAIR = new PairOfStrings();
		...
		protected void reduce(Text key, Iterable<MapWritable> values, Context context) throws IOException, InterruptedException {
			...
			for (MapWritable stripe : values) {
				for (Map.Entry<Writable, Writable> entry : stripe.entrySet()) {
					...
					int delta = ((IntWritable) entry.getValue()).get();
					Integer current = pairCounts.get(right);
					pairCounts.put(right, (current == null ? 0 : current) + delta);
				}
			}

			Integer leftTotal = word_total_map.get(key.toString());
			if (leftTotal == null || leftTotal == 0) {
				return;
			}

			for (Map.Entry<String, Integer> entry : pairCounts.entrySet()) {
				Integer rightTotal = word_total_map.get(entry.getKey());
				if (rightTotal == null || rightTotal == 0) {
					continue;
				}

				double cor = entry.getValue() / (1.0 * leftTotal * rightTotal);
				WORD_PAIR.set(key.toString(), entry.getKey());
				COR.set(cor);
				context.write(WORD_PAIR, COR);
			}
		}
	}
