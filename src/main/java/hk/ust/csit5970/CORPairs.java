...
public class CORPairs extends Configured implements Tool {
	...
	public static class CORPairsMapper2 extends Mapper<LongWritable, Text, PairOfStrings, IntWritable> {
		...
		protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			 ...
			 * TODO: Your implementation goes here.
			 */
			Set<String> sortedWordSet = new TreeSet<String>();
			while (doc_tokenizer.hasMoreTokens()) {
				String word = doc_tokenizer.nextToken();
				if (word.length() == 0) {
					continue;
			...
			if (sortedWordSet.size() < 2) {
				return;
			}

			List<String> words = new ArrayList<String>(sortedWordSet);
			for (int i = 0; i < words.size(); i++) {
				for (int j = i + 1; j < words.size(); j++) {
					WORD_PAIR.set(words.get(i), words.get(j));
	...
	public static class CORPairsReducer2 extends Reducer<PairOfStrings, IntWritable, PairOfStrings, DoubleWritable> {
		...
		protected void reduce(PairOfStrings key, Iterable<IntWritable> values, Context context) throws IOException, InterruptedException {
			...
			Integer leftTotal = word_total_map.get(key.getLeftElement());
			Integer rightTotal = word_total_map.get(key.getRightElement());
			if (leftTotal == null || rightTotal == null || leftTotal == 0 || rightTotal == 0) {
				return;
			}

			double cor = pairFreq / (1.0 * leftTotal * rightTotal);
			COR.set(cor);
