...
public class BigramFrequencyPairs extends Configured implements Tool {
	...
	private static class MyReducer extends
		...
		private float currentLeftTotal = 0.0f;
		...
		public void reduce(PairOfStrings key, Iterable<IntWritable> values,
			...
			if (!left.equals(currentLeftWord)) {
				...
				currentLeftTotal = 0.0f;
			}

			if (right.equals("")) {
				currentLeftTotal = sum;
				VALUE.set(currentLeftTotal);
				context.write(key, VALUE);
			} else {
				if (currentLeftTotal == 0.0f) {
					return;
				}
				VALUE.set(sum / currentLeftTotal);
				context.write(key, VALUE);
			}
		}
	}
